...
class KeyboardPreferences(context: Context) {
    ...
    var secureNotes: String by cachedString("SECURE_NOTES", "")
    var particleEffects: Boolean by cachedBool("PARTICLE_EFFECTS", true)
    var secureNotes: String by cachedString("SECURE_NOTES", "")
    var particleEffects: Boolean by cachedBool("PARTICLE_EFFECTS", true)
    var secureNotes: String by cachedString("SECURE_NOTES", "")
    var particleEffects: Boolean by cachedBool("PARTICLE_EFFECTS", true)
    ...
    var keyboardFontScale: Float
        get() = store.getFloat(KEYBOARD_FONT_SCALE, 1.0f)
        set(value) = store.edit().putFloat(KEYBOARD_FONT_SCALE, value).apply()

    var volumeCursor: Boolean by cachedBool("volume_cursor", false)
    var confirmExit: Boolean by cachedBool(CONFIRM_EXIT, true)
    var perAppAccent: Boolean by cachedBool("per_app_accent", false)
    var backgroundBlurRadius: Float
        get() = store.getFloat("background_blur_radius", 0f)
        set(value) = store.edit().putFloat("background_blur_radius", value).apply()
    ...
    companion object {
        ...
        private const val KEYBOARD_FONT_SCALE = "keyboard_font_scale"
        private const val CONFIRM_EXIT = "confirm_exit"
        private const val FIRST_LAUNCH_PERMS = "first_launch_perms"
        const val DEFAULT_TOOLBAR_ICONS = "lang_toggle,font_studio,singlish,case_convert,format,whatsapp,calculator,dev_mode,undo,redo,astrology,fm,translate,one_handed,emoji,clipboard,settings"
    }
}
