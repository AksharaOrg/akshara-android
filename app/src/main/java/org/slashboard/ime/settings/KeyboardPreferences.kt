...
class KeyboardPreferences(context: Context) {
    ...
    var recentEmojis: List<String>
        get() {
            ...
            val cleanedList = raw.split(",")
                .filter { it.isNotEmpty() }
                .map { it.replace("default", "", ignoreCase = true) }
                .filter { it.isNotEmpty() }
            
            // Auto-update storage if we cleaned up corrupted emojis
            if (cleanedList.joinToString(",") != raw) {
                store.edit().putString("recent_emojis", cleanedList.joinToString(",")).apply()
            }
            return cleanedList
        }
        set(value) {
